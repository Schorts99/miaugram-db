...
// Fixtures contents the image properties for the test
...
test.afterEach.always('cleanup database', async => {
  let db = t.context.db
...
test('get image', async t => {
  ...
  t.throws(db.getImage('qwerty'), /Not found/)
})
...
test('get user', async t => {
  ...
  t.deepEqual(created, result)
  t.throws(db.getUser('qwerty'), /Not found/)
})
...
test('authenticate user', async t => {
  ...
  // In the case that the user can be authenticated
  let succes = await db.authenticate(user.username, plainPassword)
  t.true(succes)
  // If the user can't be authenticated
  let fail = await db.authenticate(user.username, 'qwerty')
  t.false(fail)
  // If the user doesn't exists
  let failure = await db.authenticate('qwerty', '12345')
  t.false(failure)
})
